...
- **Notificação de CHEGADA de lead** (Parte 3 §9 + adendo autorizado): evolução da **edge `receive-lead`** — **CÓDIGO PREPARADO e commitado na feat, NÃO DEPLOYADO** (deploy de edge function só com autorização do Rubiam). Após criar o lead (só no caminho de lead NOVO, não em duplicado): consulta `user_account_access` por role `[concierge,owner,director,manager]` → (a) insere `notifications` in-app (`type='new_lead'`, `action_url='/leads'`) para todos; (b) envia **e-mail via Resend** (`noreply@nexacomercial.com.br`, padrão do projeto) invocando `send-notification-email` (service role) para cada perfil — assunto `Novo lead: {nome} · {origem}`, corpo com nome/telefone/origem/campanha + link para `/leads`; só quem tem e-mail cadastrado recebe. **Best-effort: try/catch + `Promise.allSettled` — falha de notif/e-mail NUNCA quebra a captura (o lead entra sempre).** Não exige env nova (usa `SUPABASE_URL`/`SERVICE_ROLE_KEY` já presentes; `RESEND_API_KEY` já configurada e usada por `send-notification-email`). **Deploy da `receive-lead` (`supabase functions deploy receive-lead`) AGUARDANDO autorização — PARADO antes do deploy.**
