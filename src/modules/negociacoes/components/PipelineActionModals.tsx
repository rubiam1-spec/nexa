...
function Wrap({ open, onClose, children }: { open: boolean; onClose: () => void; children: React.ReactNode }) {
  ...
  return createPortal(<div style={{ position: "fixed", inset: 0, zIndex: 9998, opacity: open ? 1 : 0, pointerEvents: open ? "all" : "none", transition: "opacity 0.2s" }}><div onClick={onClose} style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.7)" }} /><div style={mobile ? { position: "absolute", inset: 0, zIndex: 1, overflowY: "auto", WebkitOverflowScrolling: "touch" } : { position: "absolute", top: "50%", left: "50%", transform: open ? "translate(-50%,-50%)" : "translate(-50%,-48%)", zIndex: 1, width: 480, maxWidth: "95vw", transition: "transform 0.2s" }}>{children}</div></div>, document.body);
}
function Card({ children }: { children: React.ReactNode }) { const mobile = useIsMobile(); return <div style={{ background: "var(--surface-raised)", border: mobile ? "none" : "1px solid var(--border-default)", borderRadius: mobile ? 0 : 16, overflow: "hidden", boxShadow: mobile ? "none" : "0 24px 64px rgba(0,0,0,0.5)", minHeight: mobile ? "100vh" : "auto", display: mobile ? "flex" : "block", flexDirection: "column" }}>{children}</div>; }
