...
export function avancarVendaParaDocumentos(sale: Sale) {
  if (!SaleService.podeAvancarParaDocumentos(sale)) {
    throw new InvalidSaleTransitionError(
      "A venda nao pode avancar para documentacao a partir do status atual.",
    );
  }

  return SaleService.alterarStatus(sale, SaleStatus.AWAITING_DOCUMENTS);
}
